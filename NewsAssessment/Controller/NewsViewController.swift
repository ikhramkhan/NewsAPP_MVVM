...
class NewsViewController: UIViewController,UIPopoverPresentationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        setupTableView()
        getAllNews(country: "us", category: "",page: page)
    ...
    fileprivate func setupTableView() {
        ...
        newsListTableView?.configRefreshHeader(container: self, action: { [weak self] in
            self?.page = 0
            self?.getAllNews(country: "us", category: "",page: self?.page ?? 0)
        })
...
extension NewsViewController
{
    ...
    func updateLanguage(lang:String) {
        ...
        page = 0
        if (lang == "ar") {
        getAllNews(country: "ae", category: "",page: page)
        } else {
         getAllNews(country: "us", category: "",page: page)
        }
    ...
    @objc override public func didTapFilterButton(sender: UIButton) {
        ...
        vc.callBack = {(country:String,category:String) in
            if (category == "All News") {
                self.newsArray.removeAll()
                self.getAllNews(country: country.lowercased(), category: "",page: 0)
            } else {
                self.title = category.capitalized
                self.newsArray.removeAll()
                self.getAllNews(country: country.lowercased(), category: category.lowercased(),page: 0)
            }
